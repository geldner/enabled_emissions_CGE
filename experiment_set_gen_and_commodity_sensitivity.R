## GENERATION AND COMMODITY EXPERIMENTAL DESIGN - ELASTICITY SENSITIVITY ANALYSIS
## This R script runs the 4x4x4 experimental design for each of the elasticity
## sensitivity parameter files (cap_low, cap_high, type_low, type_high, both_low, both_high).
##
## For each .prm file, it runs the full experiment and saves results to separate
## subdirectories to preserve previous outputs.
##
## Output directory structure:
##   cmf_gen_and_commodity_<prm_name>/         - CMF files for each .prm variant
##   run_out_gen_and_commodity_<prm_name>/     - Run output files
##   results/experiment_gen_and_commodity_<prm_name>_*.csv - Results files
...
# NOTE: This version INCLUDES fossil_commodity shocks in the fossil parameters
renewable_params <- exp_design[renewable == TRUE]
fossil_params <- exp_design[renewable == FALSE & fuel_neutral == FALSE]  # Includes both generation AND commodity
fuel_neutral_params <- exp_design[fuel_neutral == TRUE]

cat("Found", nrow(renewable_params), "renewable parameters\n")
cat("Found", nrow(fossil_params), "fossil fuel parameters (including commodity shocks)\n")
...
cat("Total simulations to run per .prm file:", sum(!param_grid$is_no_shocks), "\n")
...
## ELASTICITY SENSITIVITY PARAMETER FILES
#########################################

# Define the .prm files to run experiments for
prm_files <- c(
  "cap_low",
  "cap_high",
  "type_low",
...
cat("========================================\n")
cat("Running experiments for", length(prm_files), "parameter file variants:\n")
...
for(prm_name in prm_files) {

  prm_file <- paste0(prm_name, ".prm")

  cat("\n########################################\n")
  cat("RUNNING EXPERIMENTS FOR:", prm_file, "\n")
  cat("########################################\n\n")

  # Check that the .prm file exists
  if(!file.exists(prm_file)) {
    cat("ERROR: Parameter file", prm_file, "not found! Skipping...\n")
    next
  }

  # Define directory names based on .prm file
  cmf_dir <- paste0('../cmf_gen_and_commodity_', prm_name)
  run_out_dir <- paste0('../run_out_gen_and_commodity_', prm_name)
  solfiles_dir <- paste0(cmf_dir, '/solfiles')

  # Base CMF template with the appropriate .prm file
  ...
  base_cmf_template <- c(
    paste0("! Experimental design CMF file (WITH commodity shocks) using ", prm_file, " !"),
    ...
    "Rest endogenous;",
    "",
    paste0("Verbal Description = Experimental design shock with commodity shocks (", prm_name, ") ;")
  )
  ...
  cat("Cleaning up old output files from", cmf_dir, "directory...\n")
  ...
  for(i in 1:nrow(param_grid)) {
    ...
    # Generate shocks for fossil fuel parameters (includes BOTH generation AND commodity shocks)
    ...
    cmf_file <- paste0(cmf_dir, '/', sim_id, '.cmf')
    writeLines(c(base_cmf_template, "", all_shocks), cmf_file)
  }

  cat("CMF files generated for", prm_name, "\n")

  ## RUN SIMULATIONS
  ##################

  cat("\nStarting simulation runs for", prm_name, "...\n")
  ...
  for(i in seq_along(sims_to_run)) {
    ...
    soldir.i <- paste0(run_out_dir, "/", sim_id)
    ...
    # Run GTAP model
    exp <- paste0('GTAPV7-EP.exe -cmf ', cmf_dir, '/', sim_id, '.cmf')
    ...
    # Progress indicator
    if(i %% 10 == 0 || i == length(sims_to_run)) {
      cat("Completed simulation", i, "of", length(sims_to_run), "for", prm_name, "\n")
    }
  }

  cat("All simulations completed for", prm_name, "!\n")

  ## EXTRACT SOLUTION DATA
  ########################

  cat("\nExtracting solution data for", prm_name, "...\n")
  ...
  for(i in seq_along(sims_to_run)) {
    ...
    ext.status <- extractvar(
      ...
      solution.out = paste0(solfiles_dir, "/", sim_id, ".sol")
    )
  ...
  cat("Reading and compiling results for", prm_name, "...\n")
  ...
  for(i in 1:nrow(param_grid)) {
    ...
    if(param_grid$is_no_shocks[i]) {
      ...
      readsol(
        ...
        paste0(solfiles_dir, "/", template_sim, "_gco2t.csv"),
        "0038"
      )
      template_results <- fread(paste0(solfiles_dir, "/", template_sim, "_gco2t.csv"))
      ...
      these_results$prm_file <- prm_name
    ...
    readsol(
      paste0(solfiles_dir, "/"),
      paste0(sim_id, ".sol"),
      paste0(solfiles_dir, "/", sim_id, "_gco2t.csv"),
      "0038"
    )

    # Load results and add metadata
    these_results <- fread(paste0(solfiles_dir, "/", sim_id, "_gco2t.csv"))
    these_results$sim_id <- sim_id
    these_results$renewable_level <- param_grid$renewable_level[i]
    these_results$fossil_level <- param_grid$fossil_level[i]
    these_results$fuel_neutral_level <- param_grid$fuel_neutral_level[i]
  ...
  # Save parameter grid for reference (with prm_file identifier)
  param_grid_copy <- copy(param_grid)
  param_grid_copy$prm_file <- prm_name
  fwrite(param_grid_copy, paste0('../results/experiment_gen_and_commodity_', prm_name, '_parameter_grid.csv'))

  # Save compiled results
  fwrite(res_table, paste0('../results/experiment_gen_and_commodity_', prm_name, '_results.csv'))

  cat("\nExperiment completed for", prm_name, "!\n")
  cat("Parameter grid saved to: ../results/experiment_gen_and_commodity_", prm_name, "_parameter_grid.csv\n", sep="")
  cat("Results saved to: ../results/experiment_gen_and_commodity_", prm_name, "_results.csv\n", sep="")

}

## COMBINE ALL RESULTS ACROSS PARAMETER FILES
#############################################

cat("\n========================================\n")
cat("COMBINING RESULTS ACROSS ALL .prm FILES\n")
cat("========================================\n")
...
for(prm_name in prm_files) {
  results_file <- paste0('../results/experiment_gen_and_commodity_', prm_name, '_results.csv')
  grid_file <- paste0('../results/experiment_gen_and_commodity_', prm_name, '_parameter_grid.csv')

  if(file.exists(results_file)) {
    all_results_list[[prm_name]] <- fread(results_file)
  }
  if(file.exists(grid_file)) {
    all_grids_list[[prm_name]] <- fread(grid_file)
  }
}

# Combine all results
if(length(all_results_list) > 0) {
  combined_results <- rbindlist(all_results_list)
  fwrite(combined_results, '../results/experiment_gen_and_commodity_sensitivity_all_results.csv')
  cat("Combined results saved to: ../results/experiment_gen_and_commodity_sensitivity_all_results.csv\n")
  cat("Total observations:", nrow(combined_results), "\n")
}

# Combine all parameter grids
if(length(all_grids_list) > 0) {
  combined_grids <- rbindlist(all_grids_list)
  fwrite(combined_grids, '../results/experiment_gen_and_commodity_sensitivity_all_parameter_grids.csv')
  cat("Combined parameter grids saved to: ../results/experiment_gen_and_commodity_sensitivity_all_parameter_grids.csv\n")
}
...
cat("\n========================================\n")
cat("ELASTICITY SENSITIVITY ANALYSIS COMPLETE\n")
cat("========================================\n\n")

cat("Experiments run for the following .prm files:\n")
for(prm_name in prm_files) {
  cat("  -", paste0(prm_name, ".prm"), "\n")
}

cat("\nDirectory structure created:\n")
for(prm_name in prm_files) {
  cat("  - cmf_gen_and_commodity_", prm_name, "/\n", sep="")
  cat("  - run_out_gen_and_commodity_", prm_name, "/\n", sep="")
}

cat("\nResults files:\n")
for(prm_name in prm_files) {
  cat("  - results/experiment_gen_and_commodity_", prm_name, "_results.csv\n", sep="")
  cat("  - results/experiment_gen_and_commodity_", prm_name, "_parameter_grid.csv\n", sep="")
}
cat("  - results/experiment_gen_and_commodity_sensitivity_all_results.csv (combined)\n")
cat("  - results/experiment_gen_and_commodity_sensitivity_all_parameter_grids.csv (combined)\n")

cat("\nTotal simulations per .prm file:", sum(!param_grid$is_no_shocks), "\n")
cat("Total simulations across all .prm files:", sum(!param_grid$is_no_shocks) * length(prm_files), "\n")
