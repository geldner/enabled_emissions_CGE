## GENERATION-ONLY EXPERIMENTAL DESIGN
...
# Note: fossil_commodity shocks are not used in experiments (they were only for baseline calibration)
renewable_params <- exp_design[renewable == TRUE]
fossil_params <- exp_design[renewable == FALSE & fuel_neutral == FALSE & fossil_commodity == FALSE]
...
base_cmf_template <- c(
  "! Experimental design CMF file using 2024 calibrated baseline !",
...
dir.create('../cmf_generation_only', recursive = TRUE, showWarnings = FALSE)

# Clear old output files from previous runs to avoid reading stale results
cat("Cleaning up old output files from cmf_generation_only directory...\n")
old_files <- list.files('../cmf_generation_only', pattern = "\\.(sl4|har|sol)$", full.names = TRUE, recursive = FALSE)
...
if(dir.exists('../cmf_generation_only/solfiles')) {
  old_solfiles <- list.files('../cmf_generation_only/solfiles', pattern = ".*", full.names = TRUE, recursive = FALSE)
...
for(i in 1:nrow(param_grid)) {
  ...
  cmf_file <- paste0('../cmf_generation_only/', sim_id, '.cmf')
...
for(i in seq_along(sims_to_run)) {
  ...
  soldir.i <- paste0("../run_out_generation_only/", sim_id)
  ...
  # Move output files to run_out_generation_only directory
  # if(file.exists(paste0(sim_id, '.sl4'))) {
  #   file.copy(paste0('../cmf_generation_only/',sim_id, '.sl4'), paste0('../run_out_generation_only/', sim_id, '/', sim_id, '.sl4'), overwrite = TRUE)
  # }
  # if(file.exists(paste0(sim_id, '-VOL.har'))) {
  #   file.copy(paste0('../cmf_generation_only/',sim_id, '-VOL.har'), paste0('../run_out_generation_only/', sim_id, '/', sim_id, '-VOL.har'), overwrite = TRUE)
  # }
  # if(file.exists(paste0(sim_id, '-SUM.har'))) {
  #   file.copy(paste0('../cmf_generation_only/',sim_id, '-SUM.har'), paste0('../run_out_generation_only/', sim_id, '/', sim_id, '-SUM.har'), overwrite = TRUE)
  # }
  # if(file.exists(paste0(sim_id, '-WEL.har'))) {
  #   file.copy(paste0('../cmf_generation_only/',sim_id, '-WEL.har'), paste0('../run_out_generation_only/', sim_id, '/', sim_id, '-WEL.har'), overwrite = TRUE)
  # }
...
dir.create('../cmf_generation_only/solfiles', recursive = TRUE, showWarnings = FALSE)
...
for(i in seq_along(sims_to_run)) {
  ...
  ext.status <- extractvar(
    solution.dir = paste0("../cmf_generation_only"),
    solution.name = paste0("/", sim_id, ".sl4"),
    solution.out = paste0("../cmf_generation_only/solfiles/", sim_id, ".sol")
  )
...
for(i in 1:nrow(param_grid)) {
  ...
  if(param_grid$is_no_shocks[i]) {
    ...
    readsol(
      paste0("../cmf_generation_only/solfiles/"),
      paste0(template_sim, ".sol"),
      paste0("../cmf_generation_only/solfiles/", template_sim, "_gco2t.csv"),
      "0038"
    )
    template_results <- fread(paste0("../cmf_generation_only/solfiles/", template_sim, "_gco2t.csv"))
  ...
  readsol(
    paste0("../cmf_generation_only/solfiles/"),
    paste0(sim_id, ".sol"),
    paste0("../cmf_generation_only/solfiles/", sim_id, "_gco2t.csv"),
    "0038"
  )

  # Load results and add metadata
  these_results <- fread(paste0("../cmf_generation_only/solfiles/", sim_id, "_gco2t.csv"))
  these_results$sim_id <- sim_id
  these_results$renewable_level <- param_grid$renewable_level[i]
  these_results$fossil_level <- param_grid$fossil_level[i]
  these_results$fuel_neutral_level <- param_grid$fuel_neutral_level[i]
...
fwrite(param_grid, '../results/experiment_generation_only_parameter_grid.csv')

# Save compiled results
fwrite(res_table, '../results/experiment_generation_only_results.csv')

cat("\nRefactored experiment completed successfully!\n")
cat("Parameter grid saved to: ../results/experiment_generation_only_parameter_grid.csv\n")
cat("Results saved to: ../results/experiment_generation_only_results.csv\n")

## SUMMARY STATISTICS
#####################

cat("\nSummary of Generation-Only Experiment:\n")
