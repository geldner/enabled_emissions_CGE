...
df = pd.read_csv('results/experiment_10x10_no_fn_gen_and_commodity_results.csv')
...
ax.set_title('Change in CO₂ Emissions - No Fuel-Neutral Adoption, Generation and Fossil Extraction\n(10×10 Grid, Each cell represents one simulation)')
...
# Find the zero level set using contour but don't draw it yet
# We'll compute it to get the segments, then draw filtered segments manually
contour = ax.contour(X_fine, Y_fine, Z_fine, levels=[0], colors='none', linewidths=0)

# Create a mask for the contour line near text positions
# Define exclusion radius around each text annotation (in data coordinates)
exclusion_radius = 2  # adjust this value to control the gap size

# Get contour line segments - contour.allsegs contains the line segments
# allsegs[i] contains segments for the i-th level
for level_idx, segments_at_level in enumerate(contour.allsegs):
    all_filtered_segments = []

    for segment in segments_at_level:
        # segment is an array of [x, y] coordinates
        current_segment = []

        for point in segment:
            # Check if this point is too close to any text position
            too_close = False
            for _, row in sim_totals.iterrows():
    ...
    # Draw the filtered contour segments
...
# Set zorder=2 to place text in front of the contour line
for _, row in sim_totals.iterrows():
...
plt.tight_layout()
plt.savefig('experiment_10x10_no_fn_gen_and_commodity_emissions.png', dpi=300, bbox_inches='tight')
