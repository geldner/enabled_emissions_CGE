## GENERATION AND COMMODITY EXPERIMENTAL DESIGN - CTAX 160 VARIANT
...
##
## CTAX 160 VARIANT: Uses basedata_ctax_160.har instead of basedata_2024.har
##
...
# Base CMF template (CTAX 160 baseline)
# Uses basedata_ctax_160.har instead of basedata_2024.har
base_cmf_template <- c(
  "! Experimental design CMF file (WITH commodity shocks) using CTAX 160 baseline !",
  ...
  "!servants = 1;",
  "",
  "! Input files - using CTAX 160 baseline:",
  "File GTAPSETS = sets.har;",
  "File GTAPDATA = basedata_ctax_160.har;",
  ...
  "Rest endogenous;",
  "",
  "Verbal Description = Experimental design shock with commodity shocks (CTAX 160) ;"
)
...
dir.create('../cmf_gen_and_commodity_ctax_160', recursive = TRUE, showWarnings = FALSE)

# Clear old output files from previous runs to avoid reading stale results
cat("Cleaning up old output files from cmf_gen_and_commodity_ctax_160 directory...\n")
old_files <- list.files('../cmf_gen_and_commodity_ctax_160', pattern = "\\.(sl4|har|sol)$", full.names = TRUE, recursive = FALSE)
...
if(dir.exists('../cmf_gen_and_commodity_ctax_160/solfiles')) {
  old_solfiles <- list.files('../cmf_gen_and_commodity_ctax_160/solfiles', pattern = ".*", full.names = TRUE, recursive = FALSE)
...
for(i in 1:nrow(param_grid)) {
  ...
  cmf_file <- paste0('../cmf_gen_and_commodity_ctax_160/', sim_id, '.cmf')
...
for(i in seq_along(sims_to_run)) {
  ...
  soldir.i <- paste0("../run_out_gen_and_commodity_ctax_160/", sim_id)
  ...
  # Run GTAP model
  exp <- paste0('GTAPV7-EP.exe -cmf ../cmf_gen_and_commodity_ctax_160/', sim_id, '.cmf')
...
dir.create('../cmf_gen_and_commodity_ctax_160/solfiles', recursive = TRUE, showWarnings = FALSE)
...
for(i in seq_along(sims_to_run)) {
  ...
  ext.status <- extractvar(
    solution.dir = paste0("../cmf_gen_and_commodity_ctax_160"),
    solution.name = paste0("/", sim_id, ".sl4"),
    solution.out = paste0("../cmf_gen_and_commodity_ctax_160/solfiles/", sim_id, ".sol")
  )
...
for(i in 1:nrow(param_grid)) {
  ...
  if(param_grid$is_no_shocks[i]) {
    ...
    readsol(
      paste0("../cmf_gen_and_commodity_ctax_160/solfiles/"),
      paste0(template_sim, ".sol"),
      paste0("../cmf_gen_and_commodity_ctax_160/solfiles/", template_sim, "_gco2t.csv"),
      "0038"
    )
    template_results <- fread(paste0("../cmf_gen_and_commodity_ctax_160/solfiles/", template_sim, "_gco2t.csv"))
  ...
  readsol(
    paste0("../cmf_gen_and_commodity_ctax_160/solfiles/"),
    paste0(sim_id, ".sol"),
    paste0("../cmf_gen_and_commodity_ctax_160/solfiles/", sim_id, "_gco2t.csv"),
    "0038"
  )

  # Load results and add metadata
  these_results <- fread(paste0("../cmf_gen_and_commodity_ctax_160/solfiles/", sim_id, "_gco2t.csv"))
  these_results$sim_id <- sim_id
  these_results$renewable_level <- param_grid$renewable_level[i]
  these_results$fossil_level <- param_grid$fossil_level[i]
  these_results$fuel_neutral_level <- param_grid$fuel_neutral_level[i]
...
dir.create('../results_ctax_160', recursive = TRUE, showWarnings = FALSE)

# Save parameter grid for reference
fwrite(param_grid, '../results_ctax_160/experiment_gen_and_commodity_parameter_grid_ctax_160.csv')

# Save compiled results
fwrite(res_table, '../results_ctax_160/experiment_gen_and_commodity_results_ctax_160.csv')

cat("\nRefactored experiment (with commodity shocks, CTAX 160) completed successfully!\n")
cat("Parameter grid saved to: ../results_ctax_160/experiment_gen_and_commodity_parameter_grid.csv\n")
cat("Results saved to: ../results_ctax_160/experiment_gen_and_commodity_results.csv\n")

## SUMMARY STATISTICS
#####################

cat("\nSummary of Generation and Commodity Experiment (CTAX 160):\n")
...
cat("Note: Using basedata_ctax_160.har as baseline data\n")
